...
def main(cfg: DictConfig):
    ...
    cuda_index = cfg.device.cuda_index
    ...
    with torch.no_grad():
        init_psnr = psnr(moving, target).mean().item()
        init_ssim = ssim(moving, target).mean().item()

    log.info(f"Initial PSNR : {init_psnr:.4f}")
    log.info(f"Initial SSIM : {init_ssim:.4f}")
    ...
    with torch.no_grad():
        ...
        log.info(
            ...
            f"  Np_list        = {cfg.model.Np_list}\n"
            f"  Nl_list        = {cfg.model.Nl_list}\n"
            f"  Nt_list        = {cfg.model.Nt_list}\n"
            f"  C_list         = {cfg.model.C_list}\n"
            f"  use_trivols    = {cfg.model.use_trivols}\n"
            f"  Nvt            = {cfg.model.Nvt}\n"
            f"  Ntt            = {cfg.model.Ntt}\n"
            f"  use_hypervol   = {cfg.model.use_hypervol}\n"
            f"  Chv            = {cfg.model.Chv}\n"
            f"  Nhv            = {cfg.model.Nhv}\n"
            f"  Nthv           = {cfg.model.Nthv}"
        )
        log.info(f"Initial DVF u: shape={u_init.shape}, max={u_init.abs().max().item():.4e}")
        del u_init
    ...
    stages = [DvfScaleStage(until_frac=s.until_frac, scale=s.scale) for s in cfg.schedule.stages]
    schedule = DvfLowresSchedule(
        stages=stages, max_iter=num_iterations, enabled=cfg.schedule.enabled
    )
    ...
    for it in pbar:
        ...
        metrics["jac"] = loss_jac.item()
        metrics["dvf_tv"] = loss_dvf_tv.item()
        metrics["lr"] = current_lr

        if it % log_every == 0 or it == num_iterations - 1:
            model.eval()
            with torch.no_grad():
                u_eval = model.materialise_dvf_shots([0])[0]
                warped_eval = warp_volume(moving, u_eval)

                psnr_val = psnr(warped_eval, target).mean().item()
                ssim_val = ssim(warped_eval, target).mean().item()
                ...
                save_checkpoint_slices(
                    os.path.join(checkpoint_dir, f"progress_{it:04d}.png"),
                    target, moving, warped_eval,
                )
                save_dvf_orthogonal_slices_png(
                    u_eval, os.path.join(dvf_dir, f"dvf_{it:04d}.png")
                )
                save_optimization_frame(
                    os.path.join(frames_dir, f"frame_{it:04d}.png"),
                    moving, target, warped_eval, loss_history,
        ...
        postfix = {
            ...
            "psnr": f"{last_psnr:.2f}" if last_psnr is not None else "—",
            "ssim": f"{last_ssim:.2f}" if last_ssim is not None else "—",
            "lr": f"{current_lr:.2e}",
        }
        pbar.set_postfix(postfix)

    with torch.no_grad():
        final_dvf = model.materialise_dvf_shots([0])[0]
        final_warped = warp_volume(moving, final_dvf)
        final_psnr = psnr(final_warped, target).mean().item()
        final_ssim = ssim(final_warped, target).mean().item()

    torch.save(final_dvf.cpu(), os.path.join(output_dir, "final_dvf.pt"))
    ...
    log_to_json(os.path.join(output_dir, "run_log.json"), log_data)

    log.info(f"Final Loss : {total_loss.item():.6f}")
    log.info(f"Final MSE  : {loss_mse.item():.6f}")
    log.info(f"Final Jac  : {loss_jac.item():.6f}")
    log.info(f"Final PSNR : {final_psnr:.4f}")
    log.info(f"Final SSIM : {final_ssim:.4f}")

    output_gif = os.path.join(output_dir, "optimization.gif")
    create_optimization_gif(frames_dir, output_gif, fps=2)
